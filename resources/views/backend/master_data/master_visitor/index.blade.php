        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between">
                ...
                <div class="btn-group">
                    ...
                    <a href="{{ route('master-visitor.create') }}" class="btn btn-dark btn-sm modal-show" title="Tambah Visitor"><i class="mdi mdi-playlist-plus"></i> Tambah</a>
                </div>
            </div>
            <div class="card-body">
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="{{ route('api.master-visitor.export') }}" method="POST" id="form-filter">
            @csrf
    ...
    function loadData(){
        table = $('#table-data').DataTable({
            ...
            ajax: {
                ...
                url: "{{ route('api.master-visitor.load-data') }}",
                type: "POST",
            ...
            columns: [
                ...
                },
                {
                    ...
                    searchable: false,
                    render: function(data, type, row, meta) {
                        let url_detail = `{{ route('master-visitor.show', ':id') }}`;
                            url_detail = url_detail.replace(':id', row.id);
                        let url_edit = `{{ route('master-visitor.edit', ':id') }}`;
                            url_edit = url_edit.replace(':id', row.id);
                        let url_delete = `{{ route('master-visitor.destroy', ':id') }}`;
                        ...
                        var btn = `
                            ...
                            <a href="${url_edit}" class="btn btn-sm btn-info p-2 modal-show edit" title="Edit Visitor"><i class="mdi mdi-lead-pencil"></i></a>
